...
// จำกัดขอบเขตให้แชทบอทตอบเฉพาะเรื่องสุนัข/บทเรียน (กันโดนใช้เป็น Gemini ทั่วไปฟรีๆ)
const ALLOWED_TOPIC_KEYWORDS = [
  // สุนัข
  "หมา", "น้องหมา", "สุนัข", "เจ้าตูบ", "พันธุ์", "เลี้ยง", "ดูแล",
  "อาหาร", "อาบน้ำ", "สุขภาพ", "พฤติกรรม", "กิน", "เห่า",
  // บทเรียน / การอ่าน
  "บทเรียน", "บทที่", "บทนี้", "chapter", "ch.", "เนื้อหา",
  "อ่าน", "หนังสือ", "บทความ", "สรุป",
  // ระบบเว็บที่ผู้ใช้อาจถามในบริบทของบทเรียน/สัตว์เลี้ยง
  "เหรียญ", "coin", "คอยน์", "เป้าหมาย", "goal", "ร้าน", "shop", "ซื้อ",
  "เกม", "game", "ด่าน", "level", "เลเวล", "สถานะ",
  "รหัสผ่าน", "password", "ช่วย", "help", "แนะนำ", "ใช้",
];

function isAllowedTopic(message) {
  const lower = message.toLowerCase();
  return ALLOWED_TOPIC_KEYWORDS.some((k) => lower.includes(k.toLowerCase()));
}
...
async function buildReply(message, context) {
  ...
  if (!isAllowedTopic(message)) {
    return "ขอโทษครับ 🐶 ผมตอบได้เฉพาะเรื่องน้องหมาและบทเรียนเท่านั้นครับ 📖🐾";
  }

  // 3) คำตอบที่แอดมินกรอกไว้ (ตอบก่อน Gemini เพราะเป็นเนื้อหาที่คนคุมเนื้อหาตั้งใจใส่มา)
